...
* The model is statistically significant. The p-value of the F-statistic is **`r summary(fit3)$coefficients[2,4]`**, which is highly significant (it is much less than 0.05), so we reject the null hyphotesis. It means that at least one predictor variable is significantly related to the outcome variable (mpg).
...
As *b1* p-value is **`r fit3$coefficients[2]`** we reject the null hypothesis (*b1* = 0). So we can conclude that manual transmission is statistical significant better than automatic transmission for fuel efficiency (mpg).
